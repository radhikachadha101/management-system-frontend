...
import CreateBidProfile from './CreateBidProfile.js';
...
function BidProfile({ getBidProfiles,profiles, profileAdded,
  getProfileById,
  profileUpdateCheck,
  profile}, filterFn) {
  const countData = profiles && profiles.length > 0 ? profiles[0].totalRecords : 0;
  const records = profiles && profiles.length > 0 ? profiles[0].data : 0;
  ...
  useEffect(() => {
    setProfileData(profile && profile.length > 0 ? profile :'')
  },[profile])

  useEffect(() => {
    if (profileAdded || profileUpdateCheck) {
      setModelOpen(false);
      getBidProfiles(offset, limit, search);
      navigate("/bidProfiles");
    }
  }, [profileAdded, getBidProfiles,profileUpdateCheck])
  ...
  const handleChange = (event, value) => {
    ...
    getBidProfiles(offsetValue, limit, search);
  };
...
export default getConnect(BidProfile);
